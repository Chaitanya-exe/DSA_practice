...
pub fn divisible_by_k(nums: Vec<i32>, k: i32) -> i32 {
    let mut prefix = HashMap::new();
    let mut sum = 0;
    let mut count = 0;
    prefix.insert(0, 1);
    for num in nums {
        sum += num;
        let remainder = ((sum % k) + k) % k;
        if let Some(freq) = prefix.get(&remainder) {
            count += freq;
        }
        *prefix.entry(remainder).or_insert(0) += 1;
    }

    count
}

pub fn max_value_after_partition(nums: Vec<i32>, k: i32) -> i32 {
    let n = nums.len();
    let mut dp = vec![0; (n+1) as usize];
...
pub fn num_decodings(s: String) -> i32 {
    let encode = s.chars().collect::<Vec<char>>();
    ...
    for i in 2..=n {
        ...
        let num = (encode[i-2] as i32 - '0' as i32) * 10 + (encode[i-1] as i32 - '0' as i32);

        if encode[i - 2] != '0' && (num >= 10 && num <= 26) {
            curr += prev2;
        }
